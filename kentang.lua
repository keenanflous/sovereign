...
local function ToggleNoNight(enabled)
    ...
    if enabled then
        ...
        Lighting.GeographicLatitude = 0  -- Equator, siang lebih lama
...
local currentSpeed = 16  -- Default speed (normal walk speed)
...
local Window = WindUI:CreateWindow({
    ...
    Size = UDim2.fromOffset(500, 500), -- Diperbesar untuk fitur tambahan
    Theme = "Dark",
    KeySystem = false
})
...
-- =======[ TEXT BOX UNTUK FOV (GANTI SLIDER) ] =======
local fovInputValue = "70"

local FOVInput = SectionCamera:TextBox({
    Title = "Field of View",
    Content = "Masukkan nilai FOV (30-120)",
    Default = "70",
    Callback = function(value)
        fovInputValue = value
        ...
        if fov and fov >= 30 and fov <= 120 then
            ...
            Notify("Error", "Nilai FOV harus antara 30-120", 3)
        end
    end
})
...
SectionCamera:Button({
    Title = "Reset FOV (70)",
    Content = "Reset FOV ke default",
    Callback = function()
        fovInputValue = "70"
        FOVInput:Set("70")
        if game.Workspace.CurrentCamera then
            game.Workspace.CurrentCamera.FieldOfView = 70
            Notify("Camera", "FOV direset ke 70", 2)
        end
    end
})

-- =======[ SECTION BARU: SPEED HACK (TEXT BOX) ] =======
...
local SpeedInput = SectionSpeed:TextBox({
    Title = "Walk Speed",
    Content = "Masukkan kecepatan (16-500)",
    Default = "16",
    Callback = function(value)
...
SectionSpeed:Toggle({
    ...
    Callback = function(value)
        ToggleSpeedHack(value, tonumber(speedInputValue) or 16)
    end
})
...
SectionSpeed:Button({
    ...
    Content = "Reset ke kecepatan normal",
    Callback = function()
        speedInputValue = "16"
        SpeedInput:Set("16")
        if speedHackEnabled then
        ...
        Notify("Speed", "Kecepatan direset ke 16", 2)
    end
})

-- =======[ SECTION BARU: JUMP POWER (TEXT BOX) ] =======
...
local function ToggleJumpHack(enabled, power)
    ...
    if enabled then
        ...
        -- Loop untuk terus mengupdate jumppower
...
local JumpInput = SectionSpeed:TextBox({
    Title = "Jump Power",
    Content = "Masukkan jump power (50-200)",
    Default = "50",
    Callback = function(value)
        ...
        if jumpPower and jumpPower >= 50 and jumpPower <= 200 then
            ...
            if jumpHackEnabled then
                ...
                Notify("Jump Hack", "Jump Power diatur ke: " .. currentJumpPower, 2)
            end
        else
            Notify("Error", "Jump Power harus antara 50-200", 3)
        end
    end
})
...
SectionSpeed:Toggle({
    ...
    Callback = function(value)
        ToggleJumpHack(value, tonumber(jumpInputValue) or 50)
    end
})
...
SectionReset:Button({
    ...
    Callback = function()
        ...
        if FOVInput then FOVInput:Set("70") end
        if SpeedInput then SpeedInput:Set("16") end
        if JumpInput then JumpInput:Set("50") end
